...
{
	s_config *config = (s_config *)obj;
	debug(LOG_INFO, "mqtt_message_callback");
...
{
	s_config 		*config = obj;
	debug(LOG_INFO, "mqtt_connect_callback");
...
void thread_mqtt(void *arg)
{
	...
	if(strlen(config->gw_id) > MOSQ_MQTT_ID_MAX_LENGTH){
		debug(LOG_ERR, "Error: MQTT id is too long");
        mosquitto_lib_cleanup();
	}
    mosq = mosquitto_new(config->gw_id, true, config);
    if( mosq == NULL ) {
		debug(LOG_ERR, "MQTT error:%s", strerror(errno));
        mosquitto_lib_cleanup();
        return ;
    }

	if(config->mqtt_server->username && config->mqtt_server->password){
		if(mosquitto_username_pw_set(mosq, config->mqtt_server->username, config->mqtt_server->password)){
	...
	/*
	if(config->mqtt_server->psk && config->mqtt_server->psk_identity){
		if(mosquitto_tls_psk_set(mosq, config->mqtt_server->psk, config->mqtt_server->psk_identity, NULL)){
   		 	mosquitto_destroy(mosq);
   		 	mosquitto_lib_cleanup();
   		    return ;
		}
