...
arp_get_ip(const char *req_mac)
{
	...
	reply = NULL;
//	while (!feof(proc) && (fscanf(proc, " %15[0-9.] %*s %*s %17[a-fa-f0-9:] %*s %*s", ip, mac) == 2)) {
	while (!feof(proc) && (fscanf(proc, " %15[0-9.] %*s %i %17[A-Fa-f0-9:] %*s %*s", ip, &flags, mac) == 3)) {
		if (strncmp(mac, req_mac, 17) == 0 && (flags & 0x02)) {
/*
...
void fw_set_block_site(const char *site)
{
	debug(LOG_DEBUG, "Blocksite");
	iptables_fw_accesscontrol(FW_ACCESS_ALLOW, site);
}

void fw_clear_block_site(const char *site)
{
	debug(LOG_DEBUG, "Clear blocksite");
	iptables_fw_accesscontrol(FW_ACCESS_DENY, site);
}
	...
	if(config->origin) {
		...
		for (p1 = p2 = worklist; NULL != p1; p1 = p2) {
			...
			if ((p1->counters.last_updated + (config->checkinterval * config->clienttimeout) <= current_time)) {
				...
				tmp = client_list_find_by_client(p1);
				if (NULL != tmp) {
					debug(LOG_DEBUG, "%s - Inactive for more than %ld seconds(%ld), removing client and denying in firewall",
						  p1->ip, config->checkinterval * config->clienttimeout, current_time);
					debug(LOG_NOTICE, "Client logout (%d)", p1->is_online);
					logout_client(tmp);
				} else {
					debug(LOG_NOTICE, "Client was already removed. Not logging out.");
				}
		...
		auth_server_request(&authresponse, REQUEST_TYPE_COUNTERS,
				NULL, NULL, NULL, 0, 0, 0, 0, 0, 0,	NULL, 0);
