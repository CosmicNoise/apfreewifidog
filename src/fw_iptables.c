...
iptables_fw_init(void)
{
	...
	iptables_do_append_command(handle, "-t mangle -N " CHAIN_ACCESS_CRONTOL);
	...
	iptables_do_append_command(handle, "-t mangle -I PREROUTING 1 -i %s -j " CHAIN_ACCESS_CRONTOL, config->gw_interface);
...
iptables_fw_destroy(void)
{
	...
	iptables_fw_destroy_mention("mangle", "POSTROUTING", CHAIN_ACCESS_CRONTOL, NULL);
	// liudf added 20160106
	...
	iptables_do_command("-t mangle -F " CHAIN_ACCESS_CRONTOL);
	// liudf added 20160106
	...
	iptables_do_command("-t mangle -X " CHAIN_ACCESS_CRONTOL);
...
int iptables_fw_accesscontrol(fw_access_t type, const char *site)
{
	int rc = 0;
	const s_config *config;
	t_auth_serv *auth_server;
	...
	switch(type){
		...
		{   
			for (auth_server = config->auth_servers; auth_server != NULL; auth_server = auth_server->next) {
				if (auth_server->last_ip && strcmp(auth_server->last_ip, "0.0.0.0") != 0) {
					debug(LOG_DEBUG, "the last ip: %s", auth_server->last_ip);
					if(iptables_do_command("-w -t mangle -C " CHAIN_ACCESS_CRONTOL " ! -s %s ! -d %s -m string --string %s --algo bm -j DROP", auth_server->last_ip, auth_server->last_ip, site)){
						rc = iptables_do_command("-w -t mangle -A " CHAIN_ACCESS_CRONTOL "! -s %s ! -d %s -m string --string %s --algo bm -j DROP", auth_server->last_ip, auth_server->last_ip, site);
						rc &= iptables_do_command("-w -t mangle -A " CHAIN_ACCESS_CRONTOL "-d %s -j DROP", site);
				}   
				}
			}
			break;
		...
		case FW_ACCESS_DENY:
			{   
				//  rc = fw_do_command("-w -t mangle -D zycfirewall_ac -m string --string %s --algo bm -j DROP", site);
				if(site){
					iptables_fw_destroy_mention("mangle", CHAIN_ACCESS_CRONTOL, site, NULL);
					iptables_do_command("-w -t mangle -D "CHAIN_ACCESS_CRONTOL " -d %s -j DROP", site);
				}   
				else {
					rc = iptables_do_command("-w -t mangle -F " CHAIN_ACCESS_CRONTOL);
				}   
				break;
			}   
		default: 
