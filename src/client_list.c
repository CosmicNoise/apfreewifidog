...
client_get_new(void)
{
	...
	client->rx = NULL; // no flow limit
	client->tx = NULL; // no flow limit
    return client;
}
...
add_online_client(const char *info)
{
	...
	if(!strcmp(ip, "0.0.0.0") && is_valid_mac(mac)){
		ip = arp_get_ip(mac);	
	}
	...
	if(is_valid_mac(mac) &&  
		...
		if(old_client == NULL) {
			...
			if(config->origin) {
				...
				char *token = "useless";
				if(config->wechat) {
					if(!strcmp(json_object_to_json_string(roam_client), "yes")){
						t_client *client = client_list_add(ip, mac, token);
						client->wired = 0;
						if (name) {
							client->name = safe_strdup(name);
					...
					json_object *devmac = NULL;
					if(json_object_object_get_ex(roam_client, "devmac", &devmac)){
						if(!strcmp(json_object_get_string(devmac), mac)){
							t_client *client = client_list_add(ip, mac, token);
							client->wired = 0;
							if (name) {
								client->name = safe_strdup(name);
