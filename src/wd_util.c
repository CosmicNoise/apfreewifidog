...
get_status_text()
{
    ...
    while (current != NULL) {
		...
		pstr_append_sprintf(pstr, "  TX: %s\n", current->name != NULL?current->tx:"null");
		pstr_append_sprintf(pstr, "  RX: %s\n", current->name != NULL?current->rx:"null");
...
{
	int rc;
	rc = execute("ubus call tc tc_flush", 1);       
	if (rc!=0) {
		debug(LOG_ERR, "tc flush rules failed(%d)", rc);    
	}
}
