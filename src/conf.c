...
const char	*g_inner_trusted_domains = "www.cellwifi.cn,zabbix.cellwifi.cn";
		...
		if (p1 && p1[0] != '\0') {
			...
			if ((strncmp(s, "#", 1)) != 0) {
				...
				case oDeltaTraffic:
					...
					break;
				case oOrigin:
					config.origin = parse_boolean_value(p1);
					debug(LOG_DEBUG, "config.origin:%d", config.origin);
					break;
				case oUsingWechat:
					config.wechat = parse_boolean_value(p1);
					debug(LOG_DEBUG, "config.wechat:%d", config.wechat);
					break;
				case oUsingWeb:
					config.web = parse_boolean_value(p1);
					break;
				case oUsingAd:
					config.ad = parse_boolean_value(p1);
					break;
				case oDaemon:
				...
				case oGatewayPort:
					...
					break;
				case oWanMACaddr:
					config.wan_mac_addr = safe_strdup(p1);
					break;
				case oAuthServer:
...
parse_boolean_value(char *line)
{
	...
	int ret = 0;
	if ((ret = strncasecmp(line, "yes", 3)) == 0) {
		return 1;
	}
	if ((ret = strncasecmp(line, "no", 2)) == 0) {
		return 0;
	}
	if ((ret = strncmp(line, "1", 1)) == 0) {
		return 1;
	}
	if ((ret = strncmp(line, "0", 1)) == 0) {
		return 0;
	}
