...
auth_server_roam_request(const char *mac)
{
	...
	snprintf(buf, sizeof(buf),
		...
		if(config->web) {
			char *content;
			size_t len;
			len = auth_request_frame(&content, NULL, mac);
			snprintf(buf, (sizeof(buf) - 1),
				"POST %s%s&routermac=%s HTTP/1.0\r\n"
				"User-Agent: WiFiDog %s\r\n"
				"Host: %s\r\n"		
				"Content-Length: %d\r\n"
			...
			char content_ad[MAX_BUF];
			snprintf(content_ad, (sizeof(content_ad) - 1), 
				"{\"OPType\":\"get\", \"Parameter\":{ \"routermac\":\"%s\", \"mac\":\"%s\"}}\r\n", "00:11:22:33:44:55", mac);
			snprintf(buf, (sizeof(buf) - 1), 
					"POST %s%s HTTP/1.0\r\n"
					"User-Agent: WiFiDog %s\r\n"
					"Host: %s\r\n"          
			...
			snprintf(buf, (sizeof(buf) - 1),
				"POST %s%s&routermac=%s&clientmac=%s&ssid=%s HTTP/1.0\r\n"
				"User-Agent: WiFiDog %s\r\n"
				"HOST: %s\r\n"
				"Content-Length: 0\r\n"
...
/*
	{
		...
		if(!getsockname(sockfd, (struct sockaddr *)&addr, &addrlen)){
			debug(LOG_ERR, "getsockname ADDR:%s port:%d", inet_ntoa(addr.sin_addr), ntohs(addr.sin_port));
		}
		else {
			debug(LOG_ERR, "getsockname:%s", strerror(errno));
		}
	}
*/
	close(sockfd);
    ...
    if ((tmp = strstr(res, "{\"")) && (end = strrchr(res, '}'))) {
		*(end+1) = '\0';
		...
		if(config->origin) {
			...
			if(config->wechat) {
				json_object *roam_jo = NULL;
				if( !json_object_object_get_ex(roam_info, "Auth", &roam_jo)){
					free(res);
					json_object_put(roam_info);
					return NULL;
				}
				/*
				char auth = NULL;	
				auth = json_object_get_string(roam_jo);
				if(auth && (strcasecmp(auth, "yes") == 0)) {
				
				}
				*/
				roam_jo = json_tokener_parse(json_object_to_json_string(roam_jo));
				free(res);
				json_object_put(roam_info);
				return roam_jo;
				...
				json_object *jso;
				json_object *c;
				if(json_object_object_get_ex(roam_info, "get", &jso)){
					if(json_object_object_get_ex(jso, "devwl", &c)){
						json_object *ret = NULL;
						if(json_object_is_type(c, json_type_array)){
							ret = json_tokener_parse(json_object_to_json_string(json_object_array_get_idx(c, 0)));
						}
						else {
							ret = json_tokener_parse(json_object_to_json_string(c));
						}
						free(res);
						json_object_put(roam_info);
						return ret;
					...
					time_t first_login, unsigned int online_time, char *name, int wired)
{
	...
	if(config->origin){
		...
		if(config->web) {
			char *content;
			size_t len;
			if(!strcmp(request_type, REQUEST_TYPE_LOGIN)){
				len = auth_request_frame(&content, ip, mac);
				if(len == 0){
					close(sockfd);
					close_auth_server();
					return AUTH_ERROR;	
			...
			else if(!strcmp(request_type, REQUEST_TYPE_LOGOUT)){
				close(sockfd);
				close_auth_server();
				return AUTH_ALLOWED;
			}
			else if(!strcmp(request_type, REQUEST_TYPE_COUNTERS)){
				len = sync_request_frame(&content);
				if(len == 0){
					close(sockfd);
					close_auth_server();
					return AUTH_ERROR;	
...
*/
	...
	if(config->origin) {
		...
		if(config->web) {
		if ((tmp = strstr(res, "{\"")) && (end = strrchr(res, '}'))) {
			authresponse->authcode = auth(ip, mac, tmp);
			return authresponse->authcode;
		}
		}
		else if(config->wechat) {
			...
			if(strstr(buf, "success") && strstr(buf, mac)){
				return (AUTH_ALLOWED);
			}
		}
