...
evhttpd_get_full_redir_url(const char *mac, const char *ip, const char *orig_url) {
	...
	if(config->origin) {
		...
		evbuffer_add_printf(evb, "%s://%s:%d%s%s&routermac=%s&clientmac=%s",
					protocol, auth_server->authserv_hostname, port, auth_server->authserv_path,
					auth_server->authserv_login_script_path_fragment, 
					config->wan_mac_addr , mac);
	}
	else if(config->wechat) {
		evbuffer_add_printf(evb, "%s://%s:%d%s%s&routermac=%s&clientmac=%s&ssid=%s", 
					protocol, auth_server->authserv_hostname, port, auth_server->authserv_path,
					auth_server->authserv_login_script_path_fragment,
					config->wan_mac_addr, mac, 
	...
	debug(LOG_INFO, "redir_url:%s", redir_url);	
	evbuffer_free(evb);
